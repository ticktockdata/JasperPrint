 ...
 * <p>
 * This class overridden in order to force the Print dialog to display on the
 * EventDispatchThread, and also to add a method to allow canceling execution.
 * <br>(Joseph A Miller, Jan. 2019)
 * <p/>
...
public class JRPrintServiceExporter extends JRAbstractExporter<PrintServiceReportConfiguration, PrintServiceExporterConfiguration, ExporterOutput, JRExporterContext> implements Printable
{
	...
	private Boolean[] printStatus;
        ...
        // variables added by JAM to allow canceling a job
	...
	protected void ensureOutput()
	{
        ...
        /**
         * Needed to overwrite this to ensure that the Print dialog shows on the
...
//				 it can be (eventually) set to true only if displayPrintDialog is true
			...
			for(reportIndex = 0; reportIndex < items.size(); reportIndex++)
			{
                            ...
                            if (canceled) return;  // added by JAM
...
//				exporter.setParameter(JRExporterParameter.FILE_RESOLVER, fileResolver);
				...
				try 
				{
					...
					if (items.size() == 1)
					{
                                            try {
                                                ...
                                                SwingUtilities.invokeLater(new Runnable() {
                                                    @Override
                                                    public void run() {
                                                        JOptionPane.showMessageDialog(null, "Doucment has no pages.", "Notice:", JOptionPane.WARNING_MESSAGE);
                                                    }
                                                });
					...
					if (displayPrintDialog)
					{
                                            ...
                                            if (SwingUtilities.isEventDispatchThread()) {
                                                isOK = printerJob.printDialog(printRequestAttributeSet);
                                            } else {
                                                try {
                                                    final PrintRequestAttributeSet atts = printRequestAttributeSet;
                                                    SwingUtilities.invokeAndWait(new Runnable() {
                                                        @Override
                                                        public void run() {
                                                            isOK = printerJob.printDialog(atts);
                                                        }
                                                    });
                                                } catch (Exception ex) {
        ...
        /**
         * Added method to enable canceling the Report.
         * Joseph A Miller, 2018-12-12
         */
